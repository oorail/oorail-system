...
#define OORAIL_PROJECT "oorail-level-crossing"
#define OORAIL_MDNS_HOSTNAME "oorail-level-crossing"
#define OORAIL_VERSION "1.1.0"

#define OORAIL_COPYRIGHT "Copyright (c) 2020 IdeaPier LLC (oorail.co.uk)"
#define OORAIL_LICENSE "GPLv3"

#define OORAIL_LEVEL_CROSSING 1

#if defined(OORAIL_LEVEL_CROSSING)
/*
 * Use GPIO 22 for the amber warning light (center)
 * Use GPIO 16 for the red right light (right)
 * Use GPIO 17 for the red left light (left)
 */
#define OORAIL_LED_LEFT 17
#define OORAIL_LED_RIGHT 16
#define OORAIL_LED_CENTER 22

/* This defines how long the warning light is lit - make it configurable */
#define OORAIL_WARNING_TIMER 3000

/* TODO:
 *  - Add support for grouping GPIOs into a single light
 *  - Add support for installations
 *  - Add support for grouping up to six lights per installation
 *  - Add support for triggering installations
 *  - Make all timers configurable via API
 *
 */
#endif
 ...
 * 
 */
#define OORAIL_FW_BUILD "2020032500"
...
SemaphoreHandle_t level_crossing;
#endif
#endif
#endif
...
#endif
#if defined(OORAIL_LEVEL_CROSSING)
  /* set pin modes for level crossing lights */
  pinMode(OORAIL_LED_LEFT, OUTPUT);
  pinMode(OORAIL_LED_RIGHT, OUTPUT);
  pinMode(OORAIL_LED_CENTER, OUTPUT);
#if defined(ESP32)
#if defined(OORAIL_ESP32_MULTICORE)
 ...
 */
#if defined(OORAIL_LEVEL_CROSSING)
  server.on("/api/1/lc/active", HTTP_GET, oorail_web_lc_active);
  server.on("/api/1/lc/clear", HTTP_GET, oorail_web_lc_clear);
#if defined(OORAIL_FEATURE_LIST_API)
  Serial.printf(" * API Endpoint: /api/1/lc/active \r\n");
  Serial.printf(" * API Endpoint: /api/1/lc/clear \r\n");
#endif
#endif
  return;
}
...
#if defined(OORAIL_LEVEL_CROSSING)
  level_crossing = xSemaphoreCreateMutex();
#endif
...
#if defined(OORAIL_LEVEL_CROSSING)
void oorail_lc_left() {
  xSemaphoreTake(level_crossing, portMAX_DELAY );
  digitalWrite(OORAIL_LED_LEFT, HIGH);
  delay(250);
  digitalWrite(OORAIL_LED_LEFT, LOW);
  delay(25);
  Serial.println("Core 1");
  xSemaphoreGive(level_crossing);
  delay(250);

  return;
}

void oorail_lc_right() {
  xSemaphoreTake(level_crossing, portMAX_DELAY );
  digitalWrite(OORAIL_LED_RIGHT, HIGH);
  delay(250);
  digitalWrite(OORAIL_LED_RIGHT, LOW);
  delay(25);
...
void oorail_lc_center() {
  xSemaphoreTake(level_crossing, portMAX_DELAY );
  digitalWrite(OORAIL_LED_CENTER, HIGH);
  delay(OORAIL_WARNING_TIMER);
  digitalWrite(OORAIL_LED_CENTER, LOW);
  delay(25);
  xSemaphoreGive(level_crossing);
  delay(250);
}
...
#if defined(OORAIL_LEVEL_CROSSING)
void oorail_oorail_web_lc_active(AsyncWebServerRequest *request) {
  /* TODO: set a variable to trigger sequence */
  request->send(200, "text/plain", "Activating Level Crossing Lights\r\n");
}

void oorail_oorail_web_lc_clear(AsyncWebServerRequest *request) {
  /* TODO: set a variable to disable sequence */
  request->send(200, "text/plain", "Clearing Level Crossing Lights\r\n");
}
#endif
